document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- INITIAL TEMPLATES FOR FAST STARTUP ---
    const initialTemplates = [ { name: "Distracted Boyfriend", url: "https://i.imgflip.com/1ur9b0.jpg" }, { name: "Drake Hotline Bling", url: "https://i.imgflip.com/30b1gx.jpg" }, { name: "Two Buttons", url: "https://i.imgflip.com/1g8my4.jpg" }, { name: "Woman Yelling at a Cat", url: "https://i.imgflip.com/3i7p.jpg" }, { name: "Change My Mind", url: "https://i.imgflip.com/24fxdx.jpg" }, { name: "UNO Draw 25", url: "https://i.imgflip.com/3l0thj.jpg" } ];
    ...
    function renderTemplates(templatesToRender) {
        ...
        templatesToRender.forEach(template => {
            ...
            img.addEventListener('click', () => {
                ...
                // Handle potential CORS issues with remote images
                image.onerror = () => { alert('This image cannot be used due to security restrictions. Please try another one.'); };
            });
            img.onerror = () => { img.style.display = 'none'; }; // Hide broken image links
    ...
    async function performUniversalSearch(searchTerm) {
        ...
        try {
            const response = await fetch(`/api/google-search?q=${encodeURIComponent(searchTerm)}`);
            ...
            gallery.innerHTML = `<p style="color: #ffcc80; text-align: center;">Search failed. The API might be busy. Please try again.</p>`;
        } finally {
